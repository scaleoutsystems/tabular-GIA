...
def run_fedsgd(cfg, global_model, criterion, attack_fn, client_dataloaders, val, test):
        ...
        with round_bar(expected_rounds_per_epoch, f"Epoch {i + 1}/{epochs}") as bar:
            while active_clients:
                ...
                for client_idx in active_list:
                    ...
                    grads = train_nostep(global_model, batch_loader, MetaSGD(lr), criterion, epochs=1)
                    ...
                    if attack_fn is not None:
                        attack_fn(global_model, batch_loader, i + 1, round_idx, client_idx)

                # 5. evaluate attack using evaluate_fn

                # 6. aggregate with fedsgd_train
                fedsgd_train(global_model, client_gradients, lr)
                #adamw_state = fedsgd_train_adamw(global_model, client_gradients, adamw_state, lr)

        # 8. epoch evaluation with validation loader
    ...
    # 9. perform final evaluation on test dataloader
...
#            state[key]["t"] = t
#
#            m.mul_(beta1).add_(agg, alpha=1.0 - beta1)
#            v.mul_(beta2).addcmul_(agg, agg, value=1.0 - beta2)
#
#            m_hat = m / (1.0 - (beta1 ** t))
#            v_hat = v / (1.0 - (beta2 ** t))
#
