...
TORCH_MODULE_CURVE_MARKERS = ["o", "s", "^", "D", "v", "*", "<", ">"]
...
def _load_run_metadata(run_dir: Path) -> dict[str, Any] | None:
    ...
    return {
        ...
        "arch": str(get_nested(cfg, "model.arch", "unknown")),
        "d_hidden": int(get_nested(cfg, "model.d_hidden")),
        "n_hidden_layers": int(get_nested(cfg, "model.n_hidden_layers")),
        "norm": str(get_nested(cfg, "model.norm")),
        "dropout": float(get_nested(cfg, "model.dropout")),
        "activation": str(get_nested(cfg, "model.activation")),
    }
...
def _collect_endpoint_rows(experiment_dir: Path, protocol_subdir: str) -> pd.DataFrame:
    ...
    for run_dir in sorted(protocol_dir.glob("run_*")):
        ...
        pareto_curve = align_attack_and_utility(attack_curve, utility_curve, metadata)
...
def _plot_endpoint_factor_panel(
    ...
    *,
    ...
    plot_styles = batch_styles(factor_rows["batch_size"].astype(int).unique().tolist())
    ...
    for col_idx, (factor_name, _, _) in enumerate(FACTOR_SPECS):
        ...
        for batch_size in sorted(plot_styles):
            color, marker = plot_styles[batch_size]
            ...
            attack_ax.errorbar(
                x,
                y_attack,
                yerr=errorbar_yerr(y_attack, y_attack_ci, bounded=True),
                fmt=f"{marker}--",
                color=color,
            ...
            bounded_utility = utility_ylim is not None and utility_ylim[0] == 0.0 and utility_ylim[1] == DEFAULT_BOUNDED_METRIC_YMAX
            utility_ax.errorbar(
                x,
                y_utility,
                yerr=errorbar_yerr(y_utility, y_utility_ci, bounded=bounded_utility),
                fmt=f"{marker}--",
                color=color,
                linewidth=PLOT_LINEWIDTH,
                markersize=PLOT_MARKERSIZE,
        ...
        apply_axis_finish(
            ...
            xlabel="",
            ylabel=ATTACK_METRIC_LABEL if col_idx == 0 else "",
    ...
    style_legend(axes_grid[0, 0].legend(title="Client Batch Size", ncol=2, frameon=True, loc="lower left"))
    fig.suptitle(
...
def _curve_markers() -> dict[tuple[str, str, float], str]:
    combos = _ordered_curve_combinations()
    return {combo: TORCH_MODULE_CURVE_MARKERS[idx % len(TORCH_MODULE_CURVE_MARKERS)] for idx, combo in enumerate(combos)}
...
def _legend_input_order_for_rowwise_display(n_items: int, ncol: int) -> list[int]:
...
def _plot_batch_split_metric_grid(
    ...
    *,
    ...
    if baseline_specs:
        ...
        legend_labels.extend(["Client Prior Baseline", "Uniform Random Baseline"])
    if legend_handles:
        display_order = _legend_input_order_for_rowwise_display(len(legend_handles), ncol=4)
        fig.legend(
            [legend_handles[idx] for idx in display_order],
            [legend_labels[idx] for idx in display_order],
            title="Activation, Normalization, Dropout",
            loc="upper center",
            bbox_to_anchor=(0.5, 1.01),
            ncol=4,
...
def main() -> None:
    ...
    for dataset_name in dataset_names:
        ...
        png_path, pdf_path = save_figure(
            fig,
            image_panels_dir / base_name,
            pdf_base_path=pdf_panels_dir / base_name,
        )
        print(png_path)
        print(pdf_path)
        ...
        for batch_size in sorted(dataset_endpoint_rows["batch_size"].astype(int).unique().tolist()):
            ...
            png_path, pdf_path = save_figure(
                tradeoff_fig,
                image_panels_dir / tradeoff_base_name,
                pdf_base_path=pdf_panels_dir / tradeoff_base_name,
            )
            print(png_path)
            print(pdf_path)
        ...
        for batch_size in sorted(dataset_attack_rows["batch_size"].astype(int).unique().tolist()):
            ...
            png_path, pdf_path = save_figure(
                attack_fig,
                image_panels_dir / attack_base_name,
                pdf_base_path=pdf_panels_dir / attack_base_name,
            )
            print(png_path)
            print(pdf_path)
            ...
            png_path, pdf_path = save_figure(
                num_fig,
                image_panels_dir / num_base_name,
                pdf_base_path=pdf_panels_dir / num_base_name,
            )
            print(png_path)
            print(pdf_path)
            ...
            png_path, pdf_path = save_figure(
                cat_fig,
                image_panels_dir / cat_base_name,
                pdf_base_path=pdf_panels_dir / cat_base_name,
            )
            print(png_path)
            print(pdf_path)
            ...
            png_path, pdf_path = save_figure(
                utility_fig,
                image_panels_dir / utility_base_name,
                pdf_base_path=pdf_panels_dir / utility_base_name,
            )
            print(png_path)
            print(pdf_path)
