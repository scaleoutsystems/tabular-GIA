...
- Strict defaults in this script:
  - strict prior by time (no same-timestamp prior relation),
  - subject-disjoint train/val/test split outputs.
  - top-K concept fitting on train split only.
- Use opt-out flags only if you explicitly want legacy behavior:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Legacy behavior: write a single CSV without subject-disjoint train/val/test split artifacts."
        ),
    )
    parser.add_argument(
        "--allow-global-topk",
    ...
    out_csv, out_yaml = build_dataset(
        ...
        subject_disjoint_split=(not bool(args.allow_subject_overlap_split)),
